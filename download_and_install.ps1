...
if(-not $USE_PROXY){
  ...
  & $PSScriptRoot\SilentInstall.ps1 -MSI_FILE_NAME $MSI_FILE_NAME -INSTALL_LOCATION $INSTALL_LOCATION -INSTANCE_URL $INSTANCE_URL -MID_USERNAME $MID_USERNAME -MID_PASSWORD $MID_PASSWORD -MID_NAME 'Silent_Install_MID' -SERVICE_ACCOUNT_NAME "$HOST_NAME\$SERVICE_ACCOUNT_NAME" -SERVICE_ACCOUNT_PASSWORD $SERVICE_ACCOUNT_PASSWORD -LOG_NAME 'Silent_Install_Log.txt' -START_MID
} else {
...
& $PSScriptRoot\validate_mid_server.ps1 -MID_NAME $MID_NAME -MID_USERNAME $MID_USERNAME -MID_PASSWORD $MID_PASSWORD
